...
var postToSpreadsheet = function(proposal, spreadsheetId, googleApiCred, callback) {
  ...
  proposal.l10nlanguageAsArray = proposal.l10nlanguage || [];
  // converting "l10nlanguage" from array to string so we can post it property to Google Sheet
  proposal.l10nlanguage = proposal.l10nlanguage && proposal.l10nlanguage.length > 0 ? proposal.l10nlanguage.join(`\n`) : ``;
...
// generate an array to push as GitHub labels
function generateGitHubLabels(proposal) {
  ...
  if (proposal.l10nlanguageAsArray && proposal.l10nlanguageAsArray.length > 0) {
    proposal.l10nlanguageAsArray.forEach(lang => {
      let l10lang = `Other`;

      for (let langKey in L10NLANGUAGE) {
        if (!L10NLANGUAGE.hasOwnProperty(langKey)) continue;

        if (lang === L10NLANGUAGE[langKey]) {
          l10lang = lang;
...
function formatGithubIssue(proposal, githubInfo, callback) {
  ...
  var milestoneId = ``;
  var labels = generateGitHubLabels(proposal);
  ...
  getGithubMilestoneId(proposal.space, githubInfo, (err, id) => {
    ...
    if (!proposal.l10nlanguage) {
      metaInBody.l10nlanguage = undefined;
    }
...
function sendConfirmationEmail(proposal, githubIssue) {
  ...
  hatchet.send(`mozfest_session_proposal_2018`, {
    ...
    request(
      ...
      (err, response, body) => {
        if (err) {
          ...
          console.log(`[Error posting to Github] Response status HTTP ${response.statusCode}.\nGithub error message: ${body}`);
