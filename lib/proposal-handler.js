...
var postToSpreadsheet = function(proposal, spreadsheetId, googleApiCred, callback) {
  ...
  var additionallanguage = proposal.additionallanguage;

  if (Array.isArray(additionallanguage)) {
    proposal.additionallanguage = additionallanguage.join(`, `);
  }
...
function formatGithubIssue(proposal, githubInfo, callback) {
  ...
  if (proposal.additionallanguage) {
    proposal.additionallanguage.split(`,`).forEach((lang) => {
      // format: "Other: language_name"
      labels.push(`Other: ${lang}`);
    });
  }
